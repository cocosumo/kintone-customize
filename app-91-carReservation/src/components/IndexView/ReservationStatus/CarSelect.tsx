...
export default function CarSelect({selectedCar,
  ...
  const options = cars.map((item)=><MenuItem key={item.carNumber} value={item.carNumber}>{`${item.store} ${item.carNumber}`}</MenuItem>);


  return (
    <Box sx={{minWidth: 220, maxWidth: 300}} >
      <FormControl fullWidth>
        <InputLabel>号車</InputLabel>
        <Select
        ...
        >
          <MenuItem value="all">全車</MenuItem>
          {options}
        </Select>
      </FormControl>
    </Box>
